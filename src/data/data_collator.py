...
class DialogueDataCollator:
    ...
    def process_one(self, prompts, return_length=False):
        total_short_context_one = 0
        truncation = TruncationStrategy.DO_NOT_TRUNCATE
        max_length = None
        prompt = prompts['input_text']
        ...
        if self.label_masking:
            ...
            schema_token_id = self.tokenizer.convert_tokens_to_ids(SQL_SPECIAL_TOKENS['schema'])
            prompter_token_id = self.tokenizer.convert_tokens_to_ids(SQL_SPECIAL_TOKENS['query'])
            assistant_token_id = self.tokenizer.convert_tokens_to_ids(SQL_SPECIAL_TOKENS['sql'])
            assert prompter_token_id >= 0 and assistant_token_id >= 0

            message_indices = []
            i = -1
            for x in flatten_message.input_ids:
                if x in (schema_token_id, assistant_token_id):
                    i += 1
